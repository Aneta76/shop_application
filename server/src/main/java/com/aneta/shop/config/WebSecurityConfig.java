...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http
                ...
                .antMatchers("/api/users/new").anonymous()
                .antMatchers("/api/orders/new").hasAnyAuthority("ADMIN", "USER")
                .antMatchers("/api/**/new").hasAnyAuthority("ADMIN")
                .antMatchers("/api/users/all").hasAuthority("ADMIN")
                .antMatchers("/api/**/all").permitAll()
                .antMatchers("/api/**/all/**").permitAll()
                .antMatchers("/api/users/update").hasAnyAuthority("ADMIN", "USER")
                .antMatchers("/api/**/update").hasAnyAuthority("ADMIN")
                .antMatchers("/api/products/categories/**").permitAll()
                .antMatchers("/api/logged-user-full-info").permitAll()
                .antMatchers("/api/orders/byUser/**").hasAnyAuthority("USER", "ADMIN")
